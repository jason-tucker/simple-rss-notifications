...
export function SinkRow({ sink }: { sink: SinkSummary }) {
  ...
  async function sendTest() {
    ...
    try {
      ...
      const body = (await res.json().catch(() => ({}))) as { ok?: boolean; error?: string; code?: string; providerMessageId?: string }
      if (res.ok && body.ok) {
        setTestMsg({ ok: true, text: `Sent — provider id ${body.providerMessageId ?? '(n/a)'}` })
      } else {
      ...
      <div className="flex items-center justify-between gap-4 p-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-zinc-500 truncate">
            {sink.type === 'smtp'
              ? <>{sink.host}:{sink.port} · {sink.username} · from {sink.from_email}{sink.use_tls === false && ' · no TLS'}</>
              : <>Resend · from {sink.from_email}</>}
          </div>
        </div>
      ...
      {testOpen && (
        <div className="border-t border-zinc-800 px-3 py-3 space-y-2">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={sendTest}
              disabled={testBusy || !testTo || sink.incomplete}
              className="rounded bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-900 hover:bg-white disabled:opacity-40"
            >
              {testBusy ? 'Sending…' : 'Send test'}
            </button>
            {sink.incomplete && <span className="text-xs text-amber-400">paste the password first</span>}
